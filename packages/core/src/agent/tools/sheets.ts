...
export function registerSheetsTool(r: ToolRegistry, deps: SheetsToolDeps): void {
  r.register({
    ...
    description:
      ...
      + "unwritable formula character), earlier cells in that same call have already been written and "
      + "saved — a partial failure is a partial write, never rolled back. Re-read before retrying, "
      + "the same as any other write-verb timeout.\n"
