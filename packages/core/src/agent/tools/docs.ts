...
export function registerDocsTool(r: ToolRegistry, deps: DocsToolDeps): void {
  r.register({
    ...
    description:
      ...
      + "does not exist still REFUSES (only writes create), and the new document only reaches disk "
      + "if the write itself succeeds — a create whose write then fails leaves no file at all, "
      ...
      + "  align, lineSpacing and style are PARAGRAPH properties: they affect every whole paragraph "
      ...
      + "file open, their tab is now holding an edit they did not ask for, and the next verb you "
      + "aim at that document (a plain read included) will save it to the file. Re-read before "
      + "doing anything else, and tell the human what may have landed — one ⌘Z takes it back. With "
      + "texts this covers all the paragraphs in the call, not one.\n"
