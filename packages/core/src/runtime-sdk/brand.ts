// R-1: Norma's own `BrandProfile` — the single object that makes every Winter-owned name in a
// spawned session say "norma" instead of "winter".
//
// WHAT A BRAND ACTUALLY DECIDES (surface map §6.5/§6.6), and why this file is load-bearing rather
// than cosmetic: the SDK derives the session HOME (`<envPrefix>HOME` → `NORMA_HOME`, `<envPrefix>
// PROFILE=dev` → `~/.norma-dev`), the transcript/memory roots under it, the temp root
...
//
// THE HOME ALIGNMENT IS THE POINT. `envPrefix: "NORMA_"` + `homeDirName: ".norma"` means
// `resolveWinterHome()` under this brand returns exactly what `resolveNormaHome()` returns —
// `NORMA_HOME` when set, `~/.norma-dev` under `NORMA_PROFILE=dev`, `~/.norma` otherwise. The
...
import type { NormaProfile } from "../profile";
 ...
 * `NORMA_PROFILE` exactly as `auth/secret-store.ts` does.
 */
export function buildNormaBrand(profile?: NormaProfile): BrandProfile {
  return {
    productName: "Norma",
    // BRAND_TOKEN_RE `/^[a-z][a-z0-9-]{0,31}$/` — the daemon package's own name.
    packageName: "norma-core",
    // DOT_DIR_RE `/^\.[a-z][a-z0-9-]{0,31}$/`. `homeDirName` is what `~/<it>` means when no
    // `NORMA_HOME` is set; `projectDirName` is the per-repo config dir (Norma's `.norma/` — the
    // CC-project-folder parity work's own directory).
    homeDirName: ".norma",
    projectDirName: ".norma",
    // INSTRUCTIONS_FILE_RE `/^[A-Z][A-Z0-9_]{0,31}\.md$/`; Norma's answer to CLAUDE.md.
    instructionsFile: "NORMA.md",
    // ENV_PREFIX_RE `/^[A-Z][A-Z0-9]{0,15}_$/` — THE TRAILING UNDERSCORE IS PART OF THE VALUE
    // (`envName(brand, "HOME")` is a bare concatenation). `"NORMA"` would refuse at `resolveBrand`
    // and, if it somehow got through, would have children reading `NORMAHOME`. The Interfaces
    ...
    // load, from `NORMA_PROFILE`. `com.norma.core.dev` under the dev profile, `com.norma.core`
    ...
    // The MCP namespace every capability tool is named under: `mcp__norma__<server>__<tool>`
    // (P8b-12). NEVER `winter` — the Mac/iOS renderers key on Norma's names.
    mcpServerName: "norma",
    ...
    // from `packageName` on purpose: it is how a user tells a Norma-spawned runtime child apart
    // from `norma-core` itself in Activity Monitor.
    processLabel: "norma-winter",
    // DELIBERATE ToS DECISION, mirrored from `providers/codex-config.ts`'s standing rule (CLAUDE.md
    // "Hard rules"): Norma self-identifies as itself. `resolveBrand` additionally REFUSES the six
    ...
    codexOriginator: "norma",
    // `/private/tmp/norma-<uid>/norma-<uid>/<projectKey>/<uuid>/{scratchpad,tasks}`.
    tempRootName: "norma",
    pluginManifestDir: ".norma-plugin",
 ...
 * including its caveat: `NORMA_PROFILE` must be set in the environment BEFORE this module is first
 ...
 */
export const NORMA_BRAND: BrandProfile = Object.freeze(buildNormaBrand());
