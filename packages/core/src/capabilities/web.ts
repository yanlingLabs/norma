...
// disallows the SDK's built-in `WebSearch`/`WebFetch`, because a built-in has neither Norma's stored
...
//
// The same three floors apply, through the same closures (Norma map §5.1 trap (ii)):
//
...
//   * **`ssrfGuard` / the dangerous-domain floor** — `web_fetch` is Norma's only sanctioned network
...
// deferral is NOT forwarded (see `computer.ts`'s note); it is recorded in `NORMA_CAPABILITY_TOOLS`
