...
describe("mode toolsets are unchanged by the derivation refactor", () => {
  ...
  // bash/task_stop/computer/send_message's own now-dispatch-deferred status) still counts as
  // "offered" — this pin is about ELIGIBILITY moving from ask_user to AskQuestion, not about
  // anything dropping off dispatch's toolset. push_notification's presence is unaffected (already
  // deferred:true, untouched by this task).
