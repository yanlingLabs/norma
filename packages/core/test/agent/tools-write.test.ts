...
describe("write tools", () => {
  test("write creates a file (and parent dirs) inside cwd", async () => {
    const { r, d } = setup();
    const res = await r.execute("write", { path: "new/dir/hello.txt", content: "norma was here" }, { cwd: d, roots: [d] });
  ...
  test("write reports true UTF-8 byte count, not char count", async () => {
    const { r, d } = setup();
    const res = await r.execute("write", { path: "u.txt", content: "héllo 🌍" }, { cwd: d, roots: [d] });
    expect(res.isError).toBe(false);
  ...
  test("edit replaces a unique occurrence; ambiguous or missing old_string errors", async () => {
    ...
    const bad = await r.execute("edit", { path: "f.txt", old_string: "two", new_string: "2" }, { cwd: d, roots: [d] });
    expect(bad.isError).toBe(true);
    expect(bad.output).toMatch(/2 occurrences/);
    const missing = await r.execute("edit", { path: "f.txt", old_string: "zzz", new_string: "x" }, { cwd: d, roots: [d] });
    expect(missing.isError).toBe(true);
    const ok = await r.execute("edit", { path: "f.txt", old_string: "one", new_string: "1" }, { cwd: d, roots: [d] });
  ...
  test("writes outside cwd are refused", async () => {
    ...
    const res = await r.execute("write", { path: "/tmp/evil.txt", content: "x" }, { cwd: d, roots: [d] });
  ...
  test("edit outside cwd is refused", async () => {
    const { r, d } = setup();
    const res = await r.execute("edit", { path: "/etc/hosts", old_string: "localhost", new_string: "x" }, { cwd: d, roots: [d] });
    expect(res.isError).toBe(true);
