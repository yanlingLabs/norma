...
export function winterExecutableForTests(): string | undefined {
  ...
  if (process.env.NORMA_WINTER_REQUIRE_BINARY === "1") throw new Error(`NORMA_WINTER_REQUIRE_BINARY=1 but NORMA_WINTER_EXECUTABLE is ${p ? "missing on disk" : "unset"} and no platform package is installed`);
