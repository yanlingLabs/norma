...
//
// The gateway enforces the SAME sixteen-method `remote` allowlist twice, once per language: here
...
// literal sixteen names, so editing ONE side without the other fails here (or in Swift) rather
...
describe("remote allowlist parity (SP2a gate G7)", () => {
  // The canonical sixteen (SP1 §6 + SP3 T4b approval.list + SP3.4 session.create +
  // session-history session.history + Chat Slice D session.setModel + Chat Slice D task 2's three
  // sync verbs) — the exact method STRINGS the Swift Gateway mirrors.
  const SIXTEEN = [
    METHODS.hello,
    METHODS.sessionList,
  ...
  test("REMOTE_ALLOWED_METHODS is EXACTLY the sixteen names", () => {
    expect(REMOTE_ALLOWED_METHODS.size).toBe(16);
    for (const m of SIXTEEN) {
      expect(REMOTE_ALLOWED_METHODS.has(m)).toBe(true);
    }
    expect([...REMOTE_ALLOWED_METHODS].sort()).toEqual([...SIXTEEN].sort());
  });

  test("the sixteen string VALUES match the Swift Gateway.remoteAllowedMethods literals", () => {
