...
export function launchdLabel(profile: WinterProfile = resolveWinterProfile()): string {
...
export function renderPlist(opts: { binaryPath: string; winterHome: string; profile?: WinterProfile }): string {
  const home = xmlEscape(opts.winterHome);
  const profile = opts.profile ?? resolveWinterProfile();
  const label = launchdLabel(profile);
  // DD branch review (I3): a launchd-installed DEV daemon must carry WINTER_PROFILE=dev into its
  // environment, or `packages/core/src/auth/secret-store.ts`'s module-load-time
  // `keychainService()` resolves to the DIST literal (`com.winter.core`) despite running out of
  // `~/.winter-dev` — silent credential cross-contamination between profiles. The dist plist must
  ...
  const envVars = profile === "dev"
    ? `<dict><key>WINTER_HOME</key><string>${home}</string><key>WINTER_PROFILE</key><string>dev</string></dict>`
    : `<dict><key>WINTER_HOME</key><string>${home}</string></dict>`;
...
<dict>
  ...
  <key>StandardOutPath</key><string>${xmlEscape(join(opts.winterHome, "logs", "core.out.log"))}</string>
  <key>StandardErrorPath</key><string>${xmlEscape(join(opts.winterHome, "logs", "core.err.log"))}</string>
</dict>
</plist>
`;
}
...
export async function installDaemon(binaryPath: string, winterHome: string): Promise<void> {
  writeFileSync(plistPath(), renderPlist({ binaryPath, winterHome }));
