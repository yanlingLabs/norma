// Pure/isolatable logic behind `winter workflow ...`, split out of main.ts so it can be
...
// output-style/model — plus WinterClient.workflowRun for `run`, which DOES need the daemon since
// that's where workflows actually execute), this file owns the argv parse decision only.

/** Parses `winter workflow`'s argv tail (everything after "workflow", i.e.
 ...
 *  other winter subcommand printing its usage rather than guessing. Mirrors model-cli.ts's
...
export function parseWorkflowArgs(argv: string[]): WorkflowCliAction {
  ...
  if (args.length === 0 || args[0] === "list") return { action: "list" }; // explicit `list` token — the usage line advertises `winter workflow [list]`
