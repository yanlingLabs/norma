...
if (import.meta.main) {
  ...
  case "sessions": {
    ...
    for (const s of sessions) console.log(`${AQUA}${s.sessionId}${RESET} ${DIM}${s.scope} · ${s.lastSeq} events${s.title ? ` — ${s.title}` : ""}${RESET}`);
    c.close();
    break;
  }
  case "send": {
  ...
  case "resume": {
    ...
    if (!sid) { // picker: list resumable sessions
      ...
      for (const r of rows) console.log(`${r.sessionId}  ${DIM}${r.scope}  ${r.lastSeq} events${r.title ? ` — ${r.title}` : ""}${RESET}`);
