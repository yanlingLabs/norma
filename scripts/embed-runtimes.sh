#!/usr/bin/env bash
# Winter Phase 8d (P8d-1/P8d-2) — the body of apple/Norma/project.yml's "Embed runtimes"
# postCompileScript, extracted into its own file so it is unit-testable standalone (project.yml
# just calls this script; see the "Embed norma-core" precedent right before it for the same
# extraction shape — that one stayed inline because it has no logic worth testing on its own,
# this one has a real gate).
#
# What it does, in order:
#   1. Release-only — same skip shape as "Embed norma-core": a Debug build never embeds either
...
#   3. Re-signs ONLY `winter` with the app's own team identity (`--identifier com.norma.winter
...
#   BUILT_PRODUCTS_DIR=<mkdtemp> CONTENTS_FOLDER_PATH=Norma.app/Contents CONFIGURATION=Release \
...
# that optional package is not installed, UNLESS NORMA_CLAUDE_REQUIRE_RUNTIME=1 is set, in which
...
# --- Step 2: stage --------------------------------------------------------------------------
# NORMA_STAGE_WINTER_PATH is a narrow TEST SEAM ONLY (embed-runtimes.test.ts) — it forwards
...
STAGE_ARGS=(--out "${DEST}")
if [ -n "${NORMA_STAGE_WINTER_PATH:-}" ]; then
  STAGE_ARGS+=(--winter "${NORMA_STAGE_WINTER_PATH}")
fi
...
codesign --force --sign "${EXPANDED_CODE_SIGN_IDENTITY}" --identifier com.norma.winter --options runtime --timestamp "${WINTER}"

WINTER_DVV="$(codesign -dvv "${WINTER}" 2>&1 || true)"
if ! echo "${WINTER_DVV}" | grep -q "^Identifier=com.norma.winter$"; then
  echo "error: winter re-sign did not land the stable identifier com.norma.winter:" >&2
  echo "${WINTER_DVV}" >&2
  exit 1
fi
...
echo "runtimes embedded + verified: winter re-signed (Identifier=com.norma.winter), claude verified untouched (TeamIdentifier=Q6L2SF6YDW, hardened runtime)"
