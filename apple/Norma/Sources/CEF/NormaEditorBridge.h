#ifndef WinterEditorBridge_h
#define WinterEditorBridge_h
...
///
/// This header exists for the same reason `WinterEditorScheme.h` does, and the requirement is stated
...
/// configuration." The two sides live in different binaries (`Sources/CEF/WinterCEF.mm` in the app,
...
/// would buy a cosmetic `window.winterQuery` at the cost of a second place for the two processes to
/// disagree, and the page does not call either one directly anyway — `bridge-protocol.js` (Task 4)
/// wraps `cefQuery` and exposes Winter's own shape to the editor. `message_size_threshold` is CEF's
...
inline CefMessageRouterConfig WinterCEFEditorBridgeRouterConfig() {
  return CefMessageRouterConfig();
}

#endif /* WinterEditorBridge_h */
