...
/// panel-cef Task 3: the runtime pin for Winter's entry point.
///
...
/// silently ignored by SwiftUI's `App` lifecycle. Winter shipped `NSPrincipalClass: NSApplication`
...
///
/// The pin is deliberately by CLASS NAME rather than `NSApp is WinterApplication`. A bridging
/// header is not a module interface — `WinterApplication` is exposed to the app target's own
/// Swift through `Support/WinterBridge.h`, but `@testable import Winter` does not re-export it, so
/// the test bundle cannot name the type. The string is what the pump report measured and is
/// exactly as load-bearing.
@MainActor
final class WinterApplicationTests: XCTestCase {
    ...
    /// — i.e. `@main` came back onto `WinterApp`, `main.swift` was renamed or dropped from the
    ...
    func testNSAppIsWinterApplicationAndNotSwiftUIsPrivateSubclass() {
        let actual = NSStringFromClass(type(of: NSApp!))
        XCTAssertEqual(
            actual, "WinterApplication",
            "NSApp is \(actual) — main.swift did not claim the NSApplication singleton before "
                + "WinterApp.main(). CEF's CefScopedSendingEvent requires NSApp to answer "
    ...
    /// C++, and deliberately confined to `WinterApplication.mm` so no CEF type reaches Swift), and
    ...
    func testNSAppConformsToCefAppProtocolSoCefScopedSendingEventCanDriveIt() {
        ...
        XCTAssertNotNil(
            ...
            "the CefAppProtocol runtime record is missing — nothing in the app declares conformance "
                + "to it any more (WinterApplication.mm's class extension)"
        )
        XCTAssertTrue(
    ...
    /// `WinterApplication` it would advertise an installation mechanism that provably does not
    /// run under SwiftUI, re-creating the precise false confidence that made the original plan
    /// wrong. `Bundle.main` under a unit test is the TEST HOST's bundle — the real `Winter.app`.
    func testAppBundleDeclaresNoPrincipalClassBecauseSwiftUIWouldIgnoreIt() {
        XCTAssertNil(
            ...
            "Winter.app declares NSPrincipalClass again. SwiftUI's App.main() ignores it, so it "
                + "cannot install WinterApplication and can only mislead; main.swift is the real "
