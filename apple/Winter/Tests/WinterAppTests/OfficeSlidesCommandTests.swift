...
/// wire -> `WinterOfficeHelper` -> `LOKBridge` -> real LOK stack) — see that file's own header for the
...
final class OfficeSlidesCommandTests: XCTestCase {
    ...
    /// left **17 `WinterOfficeHelper` processes resident**; after it, zero. Registered in the ONE
    ...
    /// ("Winter T6 Slide One/Two/Three", "first/second/third bullet") — if Tab-cycling and the
    ...
    func testLiveSlidesInfoReadsRealTitlesFromThreeSlideFixture() async throws {
        ...
        XCTAssertTrue(infoText.contains("Winter T6 Slide One"), infoText)
        XCTAssertTrue(infoText.contains("Winter T6 Slide Two"), infoText)
        XCTAssertTrue(infoText.contains("Winter T6 Slide Three"), infoText)
        ...
        XCTAssertTrue(readText.contains("Winter T6 Slide Two"), readText)
