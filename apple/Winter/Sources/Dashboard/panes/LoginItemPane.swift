...
/// `LoginItemController`, never a `WinterClient`/the controller itself.
struct LoginItemPane: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Toggle("Launch Winter at login", isOn: Binding(
                get: { enabled },
                set: { newValue in
                    enabled = newValue
                    setEnabled(newValue)
            ...
            Text("Winter starts automatically when you log in — the same setting as the menu bar's own checkbox.")
