...
///
///   1. **The unit-test host is `Winter.app` itself.** `WinterAppTests` sets
///      `TEST_HOST=…/Winter.app/Contents/MacOS/Winter`, so eager initialisation would stand
...
///   2. **Winter is a menu-bar app** (`LSUIElement`). Most launches never open a panel at all, and
...
enum WinterCEFRuntime {
    ...
    ///    `WinterCEFInitialize` refuses outright;
    ...
    /// has been shut down (`isRetryable` gates the caller, and `WinterCEFInitialize` refuses after
    ...
    static func ensureInitialized() -> Bool {
        ...
        // whole harness: no Winter-side env-var switch plumbing to ship, gate, or get wrong.
        let ok = WinterCEFInitialize(
            CommandLine.argc, CommandLine.unsafeArgv, cachePath, subprocessPath)

        if ok {
            state = .ready
        } else {
            state = .failed(String(cString: WinterCEFLastError()))
        }
        return ok
    }
    ...
    /// stylistic: a BRIDGING HEADER IS NOT A MODULE INTERFACE. `Support/WinterBridge.h` exposes
    /// `WinterCEF.h` to the app target's own Swift, but `@testable import Winter` does not re-export
    /// it, so the test bundle cannot name `WinterCEFIsInitialized` at all (`WinterApplicationTests`
    /// hit the same wall for the `WinterApplication` type and resolved it by class name). These
    /// three members are that door, and they are ordinary API rather than test-only hooks.
    static var isInitialized: Bool { WinterCEFIsInitialized() }

    /// Whether `CefShutdown` has actually run. Terminal: CEF cannot be initialised again
    /// afterwards. Never true for the never-initialised no-op below — see `WinterCEF.h`.
    static var didShutdown: Bool { WinterCEFDidShutdown() }

    /// The answer `CefLifeSpanHandler::DoClose` gives — see `WinterCEF.h` for why it must be `true`
    ...
    static var doCloseIsHandledByHost: Bool { WinterCEFDoCloseIsHandledByHost() }
    ...
    /// C function directly.
    static var popupsAreCancelled: Bool { WinterCEFPopupsAreCancelledSoCEFNeverCreatesAWindow() }
    ...
    /// Measured: it left all 18 CEF pins green. See `WinterCEF.h`.
    static var installsClickAndMenuHandlers: Bool { WinterCEFClientInstallsTheClickAndMenuHandlers() }
    ...
    /// zombie crash; see `WinterCEF.h` for what the returned object must carry and why no string
    ...
    /// closes and a renderer that never exits. See `WinterCEF.h` for what the returned object must
    ...
    static func recordAfterACloseHandsTheHostViewBack(_ hostView: NSView) -> NSObject? {
        WinterCEFRecordAfterACloseHandsTheHostViewBack(hostView)
    }
    ...
    /// interface, so the test bundle cannot name `WinterCEFExecuteCDP`) and for one more: it is the
    ...
    ///
    /// **The completion always fires** (`WinterCEF.h` carries the whole contract): synchronously with
    ...
    static func executeCDP(in container: NSView, method: String, paramsJSON: String?,
        ...
        WinterCEFExecuteCDP(container, method, paramsJSON) { ok, payload in
    ...
    /// rather than dropped?". See `WinterCEF.h` for why no needle can ask this and what the transcript
    ...
    static var pendingCDPTranscriptWithNoCEFAnywhere: String {
        WinterCEFPendingCDPTranscriptForOneBrowserWithNoCEFAnywhere()
    }
    ...
    /// `WinterCEFInitialize`'s own `NSApplicationWillTerminateNotification` subscription be
    /// unconditional, and what lets the unit-test host run the termination path at all.
    static func shutdown() { WinterCEFShutdown() }

    /// `Contents/Frameworks/Winter Helper.app/Contents/MacOS/Winter Helper` — the BASE helper.
    ...
    /// Chromium's profile directory. Bundle-id-scoped, so the dev app (`com.winter.app.dev`) and
    /// the distribution app never share one — the same dev/dist split `AppProfile` enforces for
    /// `WINTER_HOME` and the keychain service, and it matters here for a second reason: Chromium
    /// takes an exclusive lock on its profile, so two apps sharing one directory would collide.
    ///
    /// Deliberately NOT under `~/.winter` / `~/.winter-dev`: that tree is the DAEMON's, and browser
    ...
    /// harness run collides with** (editor-plumbing Task 5). `WINTER_EDITOR_HARNESS=1` starts a
    ...
    ///
    /// It is the same escape hatch, and the same rule, as `WINTER_HOME` on the daemon side: a test
    ...
    private static func rootCachePath() -> String {
        #if DEBUG
        if let override = ProcessInfo.processInfo.environment["WINTER_CEF_CACHE_PATH"],
           !override.isEmpty {
            return override
        }
        #endif
