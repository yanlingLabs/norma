#ifndef WinterApplication_h
#define WinterApplication_h

#import <AppKit/AppKit.h>

/// panel-cef Task 3: the `NSApplication` instance Winter actually runs on.
///
...
/// `SwiftUI.AppKitApplication` and silently ignores the key. Winter carried
...
/// `WinterApp.main()`, and SwiftUI then reuses this instance for the rest of the process's life.
/// `WinterApplicationTests` pins that at runtime, because nothing about it fails to compile.
///
/// **This header stays CEF-free on purpose.** It is dragged into every Swift compile through the
/// bridging header (`Support/WinterBridge.h`); the `CefAppProtocol` conformance lives in a class
/// extension inside `WinterApplication.mm`, so no CEF type ever reaches Swift.
@interface WinterApplication : NSApplication
@end

#endif /* WinterApplication_h */
