...
final class WireEnvelopeTests: XCTestCase {
    ...
    /// `WireEnvelope` at the current version — `JSONDecoder` accepts it, silently resolving the
    /// duplicate to the FIRST occurrence (measured; same for `JSONSerialization`) — so it is only
    /// rejected because the byte scan runs BEFORE the decode. Move `validateJSONShape` after the
    /// fast path "since the decode already validates" and this test, alone, is what fails.
